...
for i in lines:
    if len(re.findall('</*\w+>', i)) > 1:
        new_lines.append(re.search('</*\w+>', i)[0][1:-1] + ': ' + re.split('</*\w+>', i)[1])
    else:
        new_lines.append(re.search('</*\w+>', i)[0][1:-1] + ':')
...
file = open('main_directory/file_program_3.yaml', 'w')
